...
def initialize_centers(X, n_clusters, init, constraints=None, random_state=None):
    ...
    if isinstance(init, str):
        if INIT_KMPP in init:
            ...
            n_sub, labels = connected_components((constraints == 1), directed=False, return_labels=True)
            sub_sizes = np.bincount(labels, n_sub)
            center_ind = np.argsort(sub_sizes)[:n_clusters]
