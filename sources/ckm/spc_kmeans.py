...
class SPCKMeans:
    def __init__(self, n_clusters=2, max_iter=300, tol=1e-4, w=1, init=None, random_state=None):
    ...
    def fit(self, X, y=None, ml=[], cl=[]):
        ...
        return self.partial_fit(X, y, ml, cl)

    def partial_fit(self, X, y=None, ml=[], cl=[]):
        if self.cluster_centers_ == None:
            self._init_clusters(X)
        ...
        for iteration in range(self.max_iter):
            ...
            converged = np.allclose(difference, np.zeros(cluster_centers.shape), atol=tol, rtol=0)
