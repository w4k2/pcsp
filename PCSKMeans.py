...
class PCSKMeans(BaseEstimator, ClusterMixin):
    def __init__(self, n_clusters=2, sparsity=1.1, tol=1e-4, max_iter=20000, init_centroids=None):
        # Parameters
    ...
    def fit(self, X, const_mat=None):
        ## TODO: Consider sklearn API
        self.const_mat_ = const_mat
                ...
                for l in range(self.n_clusters):
                    for j in range(n):
                        ...
                        if self.labels_[j] != -1:
                            ...
                            if self.const_mat_[i,j] == 1 and l != self.labels_[j]:
            ...
            for i in range(d):
                for l in range(self.n_clusters):
                    ...
                    for j in range(len(cluster_indices)):
                        ...
                        for m in range(n):
                            if self.const_mat_[instance_index,m] == 1 and self.labels_[instance_index] != self.labels_[m]:
